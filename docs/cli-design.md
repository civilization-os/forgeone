...
- `trace`: 检查与清理执行轨迹
- `session`: 检查与清理待确认会话
- `tool`: 枚举当前 Runtime 已注册 Tool
- `plugin`: 枚举工作区内已声明 Plugin 清单
- `mcp`: 枚举工作区内已声明 MCP 清单
- `skill`: 枚举工作区内已声明 Skill 清单
...
- `tool list` 展示当前 `Tool Runtime` 已注册的可执行 Tool
- `plugin / mcp / skill list` 仅做工作区清单发现，不代表已接入主执行链路
- 外部扩展一旦进入执行态，仍必须通过 `Tool Runtime -> Policy Engine -> Trace` 链路
